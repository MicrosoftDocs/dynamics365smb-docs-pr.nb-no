...
description: 'Denne artikkelen beskriver de ulike måtene å arbeide med mva både manuelt og med automatisk oppsett, slik at du kan overholde land-/områdespesifikke forskrifter.'
author: brentholtorf
ms.author: bholtorf
...
Du kan beregne og vise mva-beløp i salgs- og kjøpsdokumenter på forskjellige måter. Forskjellen avhenger av hvilken type kunde eller leverandør du har å gjøre med. Du kan også endre det beregnede mva-beløpet manuelt, slik at det for eksempel samsvarer mva-beløpet beregnet av leverandøren på en gitt transaksjon.
...
Tabellen nedenfor inneholder en oversikt over hvordan salgsprisbeløpene beregnes for et salgsdokument når priser ikke er definert på **Salgspris**-siden:  
...
* Hvis du vil bruke en mva-sats for en bestemt kunde, kan du velge en gruppe i feltet **Mva-bokføringsgruppe for firma (pris)** på siden **Kundeprisgruppe**. Denne innstillingen er for eksempel nyttig når du vil at en pris skal gjelde alle kunder i et bestemt geografisk område eller en bestemt bransje.
...
Når kontantrabatten er tildelt, kan du tilbakestille den delen av kontantrabatten som gjelder mva-beløpet hvis det er beregnet en kontantrabatt på bakgrunn av et fakturabeløp med mva. Du må aktivere feltet **Juster for kontantrabatt** både i finansoppsettet generelt og i mva-bokføringsoppsettet for en bestemt kombinasjon av mva-firmabokføringsgrupper og mva-varebokføringsgrupper.  
...
Følg trinnene nedenfor for å aktivere manuelle mva-endringer på salgsdokumenter. Trinnene ligner på siden **Kjøpsoppsett**.
...
Du kan også justere mva-beløper i finanskladder, salgskladder og kjøpskladder. Du trenger for eksempel kanskje en justering hvis du angir en leverandørfaktura i kladden og det er en differanse mellom mva-beløpet som er beregnet i [!INCLUDE[prod_short](includes/prod_short.md)], og mva-beløpet på leverandørfakturaen du har mottatt.  

### <a name="to-set-the-system-up-for-manual-vat-entry-in-general-journals"></a>Slik konfigurerer du systemet for manuell mva-post i finanskladder
...
3. Når du har fullført oppsettet, kan du justere **Mva-beløp**-feltet på finanskladdelinjen, eller feltet **Motkonto-mva. - beløp** på salgs- eller kjøpskladdelinjen. [!INCLUDE[prod_short](includes/prod_short.md)] verifiserer at differansen ikke er større enn den tillatte maksimumsdifferansen.  
...
7. Opprett en kombinasjon av **Bokføringsgruppe - firma** for mva-myndighetene og **Bokføringsgruppe - vare** for import-mva.. For denne kombinasjonen velger du finanskontoen for import-mva i **Innkjøpskonto**-feltet.  

### <a name="to-create-a-new-invoice-for-the-import-authority-vendor-after-you-complete-the-setup"></a>Slik oppretter du en ny faktura for importmyndighetsleverandøren når du har fullført oppsettet
...
5. Hvis du vil at [!INCLUDE[prod_short](includes/prod_short.md)] skal opprette mottaksdeklarasjoner for bokførte følgesedler som ikke hadde en slik, merker du av for **Opprett mottaksdeklarasjoner hvis de ikke allerede er opprettet**. Når du merker av for dette alternativet, opprettes nye bekreftelser for alle bokførte følgesedler som ikke har bekreftelser i det merkede området.  
...
3. Velg det aktuelle alternativet i feltet **Status**.  
   ...
   Hvis kunden ikke returnerer den signerte leveringsbekreftelsen, velger du **Ikke mottatt**. Du må deretter sende kunden en ny faktura som inkluderer mva., fordi skattemyndighetene ikke vil godta den opprinnelige fakturaen.  

Hvis du vil vise en gruppe av bekreftelser, kan du starte fra **Leveringsbekreftelser**-siden og deretter oppdatere informasjon om status for utestående bekreftelser når du mottar dem fra kundene. Den oppdaterte informasjonen kan være nyttig når du vil søke etter alle bekreftelser som har en bestemt status, for eksempel **Påkrevd**, som du vil oppdatere til statusen **Ikke mottatt**.  
...
> [!NOTE]
> Du kan ikke opprette en ny leveringsbekreftelse i **Leveringsbekreftelse**-siden når du går til det ved hjelp av denne fremgangsmåten. Hvis du vil opprette en bekreftelse for en levering som ikke er konfigurert for å kreve en, åpner du den bokførte følgeseddelen og bruker én av de to fremgangsmåtene som er beskrevet tidligere:  
>
