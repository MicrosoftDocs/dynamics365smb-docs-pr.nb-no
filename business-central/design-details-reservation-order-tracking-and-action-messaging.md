...
description: Reservasjonssystemet er omfattende og inkluderer de sammenhengende og parallelle funksjonene i ordresporing og handlingsmeldinger.
author: brentholtorf
ms.topic: conceptual
ms.devlang: al
...
Det omfattende reservasjonssystemet inkluderer de sammenhengende og parallelle funksjonene i ordresporing og handlingsmeldinger.  

Kjernen i reservasjonssystemet er koblingen av en behovspost og en tilhørende forsyningspost, enten via reservasjon eller ordresporing. En reservasjon er en brukergenerert kobling, og en ordresporingspost er en systemgenerert kobling. Et vareantall som er angitt i reservasjonssystemet blir enten reservert eller ordresporet, men ikke begge deler samtidig. Hvordan systemene håndterer en vare, avhenger av hvordan varen er satt opp.  
...
> Reserverte antall ignoreres av planleggingssystemet, det vil si at den harde koblingen som opprettes mellom forsyning og behov, ikke kan endres gjennom planlegging.  
 ...
 Reservasjonssystemet danner også det strukturelle grunnlaget for varesporingssystemet. Hvis du vil ha mer informasjon, kan du gå til [Utformingsdetaljer: Varesporing](design-details-item-tracking.md).  
...
## Reservasjon  
 ...
 En reservasjon er en fast kobling som knytter sammen et bestemt behov og en bestemt forsyning. Denne koblingen påvirker den påfølgende lagertransaksjonen direkte og sikrer riktig utligning av vareposter for kostnadsformål. En reservasjon overstyrer standard lagermetode for en vare. Hvis du vil ha mer informasjon, kan du gå til [Utformingsdetaljer: Varesporing](design-details-item-tracking.md).  

  **Reservasjon-siden** er tilgjengelig fra alle ordrelinjer av både behovs- og forsyningstyper. På denne siden kan brukeren angi hvilken behovs- eller forsyningspost det skal opprettes en reservasjon opprette en kobling mot. Reservasjonen består av to poster med samme postnummer. Én post har et minustegn og peker til behovet. Den andre posten har et positivt fortegn og peker mot forsyningen. Disse postene lagres i tabellen *Reservasjonspost* med statusverdien *Reservasjon*. Brukeren kan vise alle reservasjonene på **Reservasjonsposter**-siden.  
...
|**Bruttobehov**|Overf.ordreseddel (ant.)|Overføringslinje|I/T|Restantall|  
 ...
 Hvis du vil ha mer informasjon, kan du gå til [Utformingsdetaljer: Tilgjengelighet på lageret](design-details-availability-in-the-warehouse.md).  
...
> [!NOTE]  
> Funksjonen for sen binding kan også brukes til å endre reservasjoner uten å informere brukeren, ved å stokke om på ikke-spesifikke reservasjoner av serie- eller partinumre. Hvis du vil ha mer informasjon, kan du gå til [Utformingsdetaljer: Varesporing og reservasjoner](design-details-item-tracking-and-reservations.md).  

### Automatiske reservasjoner  

Varen kort kan defineres til automatisk å reservere varer basert på behov, for eksempel ordrer. I slike tilfeller utførers en reservasjon mot beholdning, bestillinger, monteringsordrer og produksjonsordrer. Det gis en advarsel hvis forsyningen ikke er tilstrekkelig.  

I tillegg reserverer ulike planleggingsfunksjoner varer automatisk for å holde et behov knyttet til en bestemt forsyning. Ordresporingspostene for slike planleggingskoblinger inneholder **Reservasjon** i **feltet Reservasjonsstatus** i tabellen *Reservasjonspost* . Det opprettes automatiske reserveringer i følgende situasjoner:  

- En produksjonsordrer på flere nivåer der **Produksjonsprinsipp**-feltet for involverte overordnede og underordnede elementene er satt til **Produser til ordre**. Planleggingssystemet oppretter reservasjoner mellom den overordnet produksjonsordren og den underliggende produksjonsordren for å sikre at de behandles sammen. Denne reservasjonsbindingen overstyrer varens standard kostnadsberegning og utligningsmetode.  

- En produksjon, montering eller bestilling der feltet **Gjenbestillingsprinsipp** for den aktuelle varen er satt til **Bestilling**. Planleggingssystemet oppretter reservasjoner mellom behovet og den planlagte forsyningen for å sikre at den bestemte forsyningen opprettes. Hvis du vil ha mer informasjon, kan du gå til [Bestill](design-details-handling-reordering-policies.md#order).  
...
For forsyning eller behov som ikke er tildelt, tilordner planleggingssystemet automatisk et reservasjonsstatus **Overskudd**. Dette kan være et resultat av behov som skyldes prognoseantall eller brukerdefinerte planleggingsparametere. Dette er et legitimt overskudd, som systemet gjenkjenner, og det gir ikke opphav til handlingsmeldinger. Overskuddet kan også være ekte, overskuddstilbud eller etterspørsel som ikke spores. Dette er en indikasjon på en ubalanse i ordrenettverket, som gjør at systemet utsteder handlingsmeldinger. Vær oppmerksom på at en handlingsmelding som foreslår en endring i antallet alltid refererer til typen **Overskudd**. Hvis du vil ha mer informasjon, kan du gå til [delen Eksempel: Sporing av ordrer i delen Salg, produksjon og overføringer](#example-order-tracking-in-sales-production-and-transfers) i denne artikkelen.  

Automatiske reservasjoner som opprettes under den planlagte kjøringen, håndteres på følgende måter:  

- De gjelder for vareantall som er inkludert i tilgjengelighetsberegningen, akkurat som manuelle reservasjoner. Hvis du vil ha mer informasjon, kan du gå til delen "Utligning i reservasjoner" i denne artikkelen.  
...
> [!NOTE]  
> Ordresporingspolicyen og funksjonen Hent handlingsmeldinger er ikke integrert med prosjekter. Dette betyr at behov som er knyttet til et prosjekt, ikke spores automatisk. Siden det ikke spores, kan det føre til at bruken av en eksisterende etterfylling med prosjektinformasjon spores til et annet behov, for eksempel en ordre. Følgelig kan du støte på en situasjon der informasjonen din om tilgjengelig beholdning ikke er synkronisert.  

### Ordrenettverket  

Ordresporingssystemet er basert på prinsippet om at ordrenettverket alltid må være balansert, slik at hvert behov som kommer inn i systemet, kompenseres av en tilsvarende forsyning og omvendt. Systemet sørger for dette ved å identifisere logiske koblinger mellom alle behovs- og forsyningsposter i ordrenettverket.  
...
Ordresporingspostene for de to partiene for komponenten som gjenspeiler behovet i tabell 5407, er endret fra reservasjonsstatusen *Sporing til* Overskudd *·*. Årsaken er at forsyningene som var knyttet til tidligere, i tabell 32, brukes ved forsendelsen av overføringsordren.  

Ekte overskudd, som i dette tilfellet gjenspeiler overflødige forsyning eller behov som forblir ikke-sporet. Det er en indikasjon på en ubalanse i ordrenettverket, som genererer en handlingsmelding fra planleggingssystemet med mindre det løses dynamisk.  
...
Siden komponentbehovet ble endret til *WEST-lokasjon*, og forsyningen er tilgjengelig som vareposter på *WEST-lokasjonen*, blir alle ordresporingsposter for de to partinumrene nå fullstendig sporet, angitt av reservasjonsstatusen *Sporing*.  

Partiet **nr.** -feltet er nå fylt ut i ordresporingsposten for tabell 5407 fordi partinumrene ble tilordnet produksjonsordrekomponentlinjene.  

## Handlingsmeldinger  

Når sporingssystemet oppdager en ubalanse i ordrenettverket, oppretter det automatisk en handlingsmelding for å varsle brukeren. Handlingsmeldinger er systemgenererte oppfordringer til brukerhandling som spesifiserer detaljene i ubalansen og forslag til hvordan du gjenoppretter balansen i ordrenettverket. De vises som planleggingslinjer på **siden Planleggingsforslag når du velger** handlingen Hent **handlingsmeldinger** . I tillegg genereres handlingsmeldinger på planleggingslinjer under planleggingskjøringen for å gjenspeile planleggingssystemets forslag til å gjenopprette balansen i ordrenettverket. I begge tilfeller kjøres forslagene på ordrenettverket når du velger **handlingen Utfør handlingsmelding** .  
...
Sporingssystemet prøver alltid å løse en ubalanse i det eksisterende ordrenettverket. Hvis dette ikke er mulig, utsteder det en handlingsmelding for å opprette en ny ordre. Følgende er en prioritert liste som ordresporingenssystemet bruker når det avgjør hvordan balansen skal gjenopprettes. Hvis et ekstra behov kommer inn i ordrenettverket, søker systemet ordresporing gjennom følgende kontroller:  

1. Se etter eventuell overflødig forsyning i eksisterende ordresporingspost for dette behovet.  
1. Se etter planlagte og tidsplanlagte mottak sortert etter mottaksdato. Den senest mulige datoen velges.  
1. Se etter tilgjengelige beholdning.  
1. Kontroller om det finnes en forsyningsordre i gjeldende sporingspost. I så fall utsteder systemet en handlingsmelding av typen *Endre* for å øke ordren.  
1. Pass på at det ikke finnes en forsyningsordre i gjeldende sporingspost. I så fall utsteder systemet en handlingsmelding av typen *Ny* for å opprette en ny ordre.  
...
### Planleggingssystem kontra sporing og handlingsmeldinger  
