---
title: 'Tell, juster og klassifiser lagerbeholdning på nytt'
...
## <a name="to-count-physical-inventory"></a>Vareopptelling

Tell den faktiske varebeholdningen for å se om antallet som er registrert, er identisk med det fysiske antallet som er på lager. Det vil si å telle de faktiske varene på lager. Vanligvis skjer opptelling på slutten av et regnskapsår, men enkelte bedrifter teller varer oftere. Hvis det er en differanse, må de faktiske antallene bokføres til varekontiene før du kan fastsette verdien av lageret.
...
- Skriv ut rapporten du vil bruke for tellingen.
...
### <a name="to-calculate-expected-inventory-in-basic-warehouse-configurations"></a>Slik beregner du den forventede beholdningen i enkle lagerkonfigurasjoner
...
2. Velg handlingen **Beregn beholdning** fra **Forbered**.
...
1. På **siden Vareopptellingskladder** som inneholder beregnet forventet beholdning, velger du handlingen **Skriv ut** på **Hjem**.
2. På **siden Liste** over opptelling angir du om rapporten skal vise beregnet antall og lagervarer sortert etter serie- og partinumre.
...
1. På hver linje på **siden Vareopptellingskladder** der den faktiske beholdningen som er beregnet av det fysiske antallet, avviker fra det beregnede antallet, angir du den faktiske beholdningen **i feltet Antall (fysisk beholdning)** .
...
2. Hvis du vil justere de beregnede antallene til de faktiske opptalte antallet, velger du Bokfør-handlingen **på** **Hjem**.

    Bokføring oppretter vareposter og fysiske vareopptellingsposter. Åpne siden Varekort for å vise varen for å finne vareopptellingspostene for varen.
...
4. Du kontrollerer antallet ved å åpne **siden Vare kort**  for varen og velge handlingen **Vareopptellingsposter** fra **Vare**
1. .
...
3. Definer filtrene for å angi varene som skal telles i kladden, og klikk deretter på **OK**.
   ...
   Hvis du bare teller en vare i noen hyller, men ikke i andre, kan du angi differanser og bokføre dem senere på siden **Varekladd** ved hjelp av handlingen **Beregn lagerjustering**.  
...
Når du registrerer vareopptelling, bokfører du ikke til varen, vareopptellingen eller verdipostene. Postene er imidlertid tilgjengelige for avstemming når det er nødvendig. Hvis du vil beholde antall nøyaktige etter at du har talt varene i alle hyller, bokfører du resultatet.<!--physical inventory journal--> Finn ut mer under [Synkroniser antall i vareposten og på lageret](inventory-how-count-adjust-reclassify.md#to-synchronize-the-adjusted-warehouse-entries-with-the-related-item-ledger-entries).

## <a name="to-do-cycle-counting"></a>Utfør syklustelling

Du kan telle varer så ofte du vil. For eksempel fordi de er mer verdifulle eller bestselgere. Angi opptellingsintervallet ved å tildele spesielle opptellingsperioder til varene.
...
### <a name="to-start-a-count-based-on-counting-periods-in-basic-warehouse-configurations"></a>Slik starter du en opptelling basert på opptellingsperioder i enkle lageroppsett
...
### <a name="to-start-a-count-based-on-counting-periods-in-advanced-warehouse-configurations"></a>Slik starter du en opptelling basert på opptellingsperioder i avanserte lageroppsett
...
## <a name="to-adjust-the-quantity-of-one-item"></a>Juster antallet for én vare
...
### <a name="to-adjust-the-quantities-of-multiple-items-in-basic-warehouse-configurations"></a>Slik justerer du antall for flere varer i enkle lageroppsett

På **Varekladder-siden** kan du bokføre varetransaksjoner direkte for å justere lagerbeholdningen for kjøp, salg og positive eller negative endringer uten å bruke dokumenter.
...
5. Angi et positivt antall i feltet **Antall** hvis du finner ekstra varer. Hvis det mangler varer, angir du et negativt antall.  
...
Bokfør justeringshyllepostene i varepostene for de definerte periodene. Enkelte selskaper bokfører daglige justeringer i vareposten, mens andre avstemmer sjeldnere.
...
4. På hurtigfanen **Alternativer** angir du et nummer i feltet **Bilagsnummer**. Nummeret vises på varekladdlinjene.
5. Velg **OK**. De positive og negative justeringene summeres for hver vare, og linjer opprettes i varekladden.  
6. Bokfør kladdelinjene for å angi antallsavviket i vareposten. Nå samsvarer antallene i hyllene og vareposten.  
