...
### Finjustere søket med filterkriterier
...
### Definere filtre i rapporter, kjørsler og XMLport-er
...
|`Bicycle..Car`| Strenger Sykkel gjennom Bil når den bestilles leksiografisk|  
...
> [!NOTE]  
> Hvis feltet du filtrerer på, er av typen Tekst, brukes leksiografisk rekkefølge for å bestemme hva som er inkludert i intervallet. For slike felter som brukes til å lagre heltall, kan dette føre til det (uventede) resultatet at et filter på 10000..10042 også inkluderer verdiene 100000 og 1000042.
...
#### (*) Et ubegrenset antall ukjente tegn
...
### <a name="symbols"></a>Filtrere etter verdier som inneholder symboler
...
#### (%me eller %user) Poster som er tildelt til deg

Bruk `%me` eller `%user` når du filtrerer felt som inneholder bruker-ID-en, for eksempel feltet **Tilordnet til bruker-ID**, for å vise alle postene som er tilordnet til deg.

|Eksempel|Viste poster|  
|-----------------------|-----------------------|  
|`%me`<br />eller<br />`%user`|Poster som er knyttet til brukerkontoen. |  

#### (%mycustomers) Kunder i Mine kunder
...
#### (%myvendors) Leverandører i Mine leverandører
