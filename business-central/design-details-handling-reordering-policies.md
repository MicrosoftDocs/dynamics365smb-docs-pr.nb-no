...
---
# <a name="design-details-handling-reordering-policies"></a>Utformingsdetaljer: Håndter gjenbestillingsprinsipper
...
## <a name="monitoring-the-projected-inventory-level-and-the-reorder-point"></a>Overvåking av forventet lagernivå og gjenbestillingspunkt
...
### <a name="projected-inventory"></a>Beregnet beholdning
...
* Når en ny forsyningsordre blir introdusert, kontrolleres det om det ble angitt før nåværende forsyning. Hvis det er tilfelle, blir den nye forsyningen gjeldende forsyning og balanseringen starter på nytt.  
...
Formålet med tidsperioden er å samle inn behovshendelser i en tidsperiode for å opprette en felles forsyningsordre.  
...
### <a name="calculating-the-overflow-level"></a>Beregn overflytnivået
...
### <a name="creating-the-planning-line-with-an-overflow-warning"></a>Opprett planleggingslinje med en overflytadvarsel
...
#### <a name="calculating-the-planning-line-quantity"></a>Beregn antall for planleggingslinjen
...
> [!NOTE]  
> Som med alle advarselslinjer, blir maksimum og minimum ordreantall og bestillingsfaktor ignorert.  

#### <a name="defining-the-action-message-type"></a>Angi handlingsmeldingstypen
...
#### <a name="composing-the-warning-message"></a>Skriv advarselsmeldingen
...
|-----------------------|------------------|  
|Maksimalt ordreantall|100|  
|Gjenbestillingspunkt|50|  
|Lager|80|  

#### <a name="situation-before-a-sales-decrease"></a>Situasjonen før en salgsreduksjon
...
## <a name="handling-projected-negative-inventory"></a>Håndter forventet negativ lagerbeholdning
...
### <a name="fixed-reorder-quantity"></a>Fast gjenbestillingsantall
...
Forsyningsordrer som utelukkende opprettes for å dekke et gjenbestillingspunkt, utelates fra forsyningsbalansering, og blir ikke endret etterpå. Hvis du vil sette opp en vare som har et gjenbestillingspunkt, kan du se gjennom de utestående forsyningsordrene manuelt eller endre gjenbestillingsprinsippet til **Parti for parti**. Systemet reduserer eller avbryter ekstra forsyning.  

#### <a name="combines-with-order-modifiers"></a>Kombineres med ordreendringer
...
#### <a name="shouldnt-be-used-with-forecasts"></a>Skal ikke brukes med prognoser
...
Hvis du reserverer et antall, for eksempel et antall på lager for fremtidig behov, forstyrrer du kanskje planleggingsgrunnlaget. Selv om det beregnede beholdningsnivået er akseptabelt i relasjon til gjenbestillingspunktet, er antallene kanskje ikke disponible. Systemet kan forsøke å kompensere ved å opprette unntaksordrer. Vi anbefaler imidlertid at **Reserver**-feltet er satt til **Aldri** på varer som er planlagt med et gjenbestillingspunkt.

### <a name="maximum-quantity"></a>Maksimumsantall
...
#### <a name="calculate-per-time-bucket"></a>Beregn per tidsperiode
...
#### <a name="combine-with-order-modifiers"></a>Kombiner med ordremodifikatorer

Avhengig av oppsettet kan det være best å kombinere policyen for maksimumsantall med ordremodifikatorer:

* Sikre et minimums bestillingsantall
...
### <a name="combine-with-calendars"></a>Kombiner med kalendere
...
På noen måter ligner Parti-for-parti-prinsippet på ordrepolicyen. Den kan godta antall i beholdningen, og det pakker behov og forsyning i tidsperiodene du definerer.
...
Tidsperioden begrenses også når du skal planlegge en forsyningsordre på nytt for å oppfylle et gitt behov. Forsyningen innenfor tidsperioden planlegges på nytt inn eller ut for å dekke behovet. Tidligere forsyning fører til ekstra lager, og du bør avbryte den. Opprett en ny forsyningsordre for å få en senere forsyning.  

Med denne policyen kan du angi et sikkerhetslager for å kompensere for endringer i forsyningen eller for å dekke et plutselig behov. Parti-for-parti-prinsippet kan også inneholde en avdempingsperiode og et avdempingsantall for å redusere ordreplanleggingen.  

Sammen med feltet **Periode for ny planlegging** bidrar feltet **Akkumuleringsperiode for parti** til å definere gjenbestillingssyklusen. Fra datoen for første behov akkumulerer systemet alle behovene i neste akkumuleringsperiode for parti i én forsyningsordre på datoen for første behov. Behov som er utenfor akkumuleringsperioden for partiet, dekkes ikke av forsyningsordren.
