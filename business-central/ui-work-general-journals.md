...
De fleste kladder er basert på finanskladden, og du kan behandle alle transaksjoner på **siden Finanskladder** . Finn ut mer på [Bokfør transaksjoner direkte i Finans](finance-how-post-transactions-directly.md).  
...
Du kan for eksempel bruke Bokfør ansattutgifter for refusjon. Finn ut mer på [Registrere og refundere ansattes utgifter](finance-how-record-reimburse-employee-expenses.md).
 ...
 [!INCLUDE [prod_short](includes/prod_short.md)] Tilbyr imidlertid også kladder som er optimalisert for bestemte typer transaksjoner, for eksempel betalingskladden *for* registrering av betalinger. Finn ut mer under [Registrere betalinger og refusjoner i utbetalingskladden](payables-how-post-payments-refunds.md).  
...
Det finnes flere finanskladdemaler. Hver kladdemal representeres av en egen side med bestemte funksjoner og feltene som kreves for å støtte disse funksjonene, for eksempel **siden Betalingsavstemmingskladder** for å behandle bankbetalinger og **siden Utbetalingskladder** for å betale leverandørene eller refundere de ansatte. Finn ut mer under [Foreta betalinger](payables-make-payments.md) og [Avstem kundebetalinger med innbetalingskladden eller fra kundeposter](receivables-how-apply-sales-transactions-manually.md).
...
Du kan aktivere en bakgrunnskontroll som bidrar til å forhindre forsinkelser ved bokføring. Sjekken gir deg beskjed når en feil i finanskladden du arbeider på, hindrer deg i å bokføre kladden. På **siden Finanskladder** kan du velge **Feilkontroll** i bakgrunnen for å få [!INCLUDE[prod_short](includes/prod_short.md)] deg til å validere finanskladder, for eksempel finanskladder eller betalingskladder, mens du arbeider med dem.

Når du aktiverer valideringen, viser faktaboksene **Kontroller dokument** eller **Kladdkontroll** på nåværende linje og i hele kladden. Valideringen skjer når du laster inn en finanskladd, og når du velger en annen kladdelinje. Flisen **Totalt antall problemer** i faktaboksen viser totalt antall problemer som [!INCLUDE[prod_short](includes/prod_short.md)] har funnet, og du kan velge den for å åpne en oversikt over problemene.
...
På **siden Gjentakende finanskladder** kan du velge handlingen **Fordelinger** for å angi hvordan beløp på gjentakelseskladdelinjen skal fordeles på flere konti og dimensjoner. Tildeling fungerer som motkontolinje til gjentakende kladdelinje.
...
Den følgende fremgangsmåten er basert på **siden Finanskladder**, men gjelder for alle andre kladder som er basert på finanskladden, for eksempel **siden Utbetalingskladder** .
