...
description: 'Denne artikkelen gir veiledning for identifisering, feilsøking og løsning av synkroniseringsfeil.'
author: brentholtorf
ms.topic: conceptual
ms.devlang: al
...
---
# <a name="troubleshoot-synchronization-errors"></a>Feilsøk synkroniseringsfeil

Mange bevegelige deler er involvert i integrasjon av [!INCLUDE[prod_short](includes/prod_short.md)] med [!INCLUDE[prod_short](includes/cds_long_md.md)], og noen ganger går det galt. I denne artikkelen omtales noen av de typiske feilene som oppstår, og det oppgis noen punkter for hvordan de skal løses.

Feil oppstår ofte enten på grunn av noe en bruker gjorde med koblede poster, eller fordi noe er galt med hvordan integrasjonen er konfigurert. Ved feil relatert til koblede poster kan brukere løse disse selv. Årsaker til feilene er ofte handlinger som sletting av data i én bedriftsapp, men ikke i begge bedriftsappene, og påfølgende synkronisering. Hvis du vil ha mer informasjon, kan du se [Vise statusen for en synkronisering](admin-how-to-view-synchronization-status.md).
...
|Tilkoblingen til Dataverse stanset midlertidig under synkronisering. For eksempel fordi økten ble tidsavbrutt. Når det skjer, vises følgende feil når du gjenopptar økten: _Tabelltilkobling for tabelltype CRM må registreres ved hjelp av RegisterTableConnection eller cmdleten New-NAVTableConnection før den kan brukes._|* For tidsavbrudd for økter oppdaterer du siden du vil koble til Dataverse, på nytt. Feilmeldingen forsvinner.<br><br>* Hvis det er et problem i koden, for eksempel hvis du bruker en egendefinert side til å vise data fra en Dataverse-enhet, kontakter du Microsoft-partneren eller -kundestøtte. Når du gjør det, bruker du handlingen **Kopier detaljer** til å dele detaljer om feilen. |
...
* Feltene **Kilde** og **Destinasjon** kan inneholde koblinger til raden der feilen ble funnet. Velg koblingen for å undersøke feilen.  
* Handlingene **Slett oppføringer eldre enn 7 dager** og **Slett alle oppføringer** rydder opp i listen. Vanligvis bruker du disse handlingene etter at du løser årsaken til en feil som påvirker mange poster. Du må imidlertid være forsiktig. Disse handlingene kan slette feil som fremdeles er relevante.
