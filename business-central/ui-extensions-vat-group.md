...
MVA-gruppemedlemmer og grupperepresentanten kan bruke den assisterte oppsettsveiledningen for **Oppsett av mva-gruppestyring** til å definere samarbeidet med gruppen, og opprette en forbindelse mellom deres [!INCLUDE[d365fin](includes/d365fin_md.md)]-leietakere. Gruppemedlemmene bruker tilkoblingen til å sende omsetningsoppgavene deres til grupperepresentanten. Representanten sender mva til skattemyndighetene på vegne av gruppen med én enkelt omsetningsoppgave. 
...
[!INCLUDE[d365fin](includes/d365fin_md.md)] støtter sendinger av omsetningsoppgaver internt i gruppen for selskaper som bruker [!INCLUDE[d365fin](includes/d365fin_md.md)] lokalt eller online, og i en hvilken som helst kombinasjon. Oppsettet av kommunikasjonen avhenger av konstellasjonen. Følgende avsnitt beskriver hvordan du definerer ulike gruppekonstellasjoner.
...
2. Del de tekniske opplysningene som mva-gruppemedlemmer og representanten må ha for å koble sine [!INCLUDE[d365fin](includes/d365fin_md.md)]-leietakere.

  Vanligvis har mva-grupperepresentanten denne informasjonen, for eksempel navnet på miljøet for mva-grupperepresentanten som mva-gruppemedlemmene skal sende mva til.
3. Opprett brukere i mva-grupperepresentantens [!INCLUDE[d365fin](includes/d365fin_md.md)] som mva-gruppemedlemmer kan bruke til å godkjenne og koble til.
...
5. Definer utvidelsen for mva-gruppestyring i mva-grupperepresentantens [!INCLUDE[d365fin](includes/d365fin_md.md)] ved hjelp av den assisterte oppsettsveiledningen for **Konfigurer mva-gruppestyring**. 

> [!NOTE]
> For å kunne koble til mva-grupperepresentanten trenger gruppemedlemmer en bruker med tilgang til mva-grupperepresentantens [!INCLUDE[d365fin](includes/d365fin_md.md)]. MVA-grupperepresentanten må opprette minst én bruker for dette, men av sikkerhetsmessige grunner anbefales det at du oppretter en bruker for hvert enkelt mva-gruppemedlem. Pass på at du distribuerer legitimasjonen for disse brukerne til mva-gruppemedlemmer på en sikker måte.

## <a name="understanding-the-vat-group-management-setup"></a>Forstå oppsettet av mva-gruppestyring

Mva-grupperepresentanten viser en API som mva-gruppemedlemmene kan bruke til å koble seg til sin [!INCLUDE[d365fin](includes/d365fin_md.md)]-leietaker og sende omsetningsoppgaver. Mva-gruppemedlemmer vil ofte bruke [!INCLUDE[d365fin](includes/d365fin_md.md)] i separate Azure Active Directory-leietakere. Derfor trengs det mer oppsett for å opprette en forbindelse mellom mva-gruppemedlemmet og -representantens [!INCLUDE[d365fin](includes/d365fin_md.md)]. 
...
Når mva-grupperepresentanten bruker [!INCLUDE[d365fin](includes/d365fin_md.md)] online eller lokalt, bruker mva-gruppemedlemmene Azure Active Directory til å godkjenne når de sender omsetningsoppgaver til mva-grupperepresentanten. Hvis medlemmene i mva-gruppen også bruker [!INCLUDE[d365fin](includes/d365fin_md.md)] online, kan mva-gruppemedlemmet godkjenne ved å bruke den angitte brukerlegitimasjonen og informasjonen om endepunkt. Se [Oppsett av mva-gruppemedlem](ui-extensions-vat-group.md#vat-group-member-setup) hvis du vil ha mer informasjon.

Mva-gruppemedlemmer som har [!INCLUDE[d365fin](includes/d365fin_md.md)] lokalt, må konfigurere en appregistrering i Azure Active Directory for mva-grupperepresentantens [!INCLUDE[d365fin](includes/d365fin_md.md)]-leietaker. Appregistreringen gjør at mva-grupperepresentantens [!INCLUDE[d365fin](includes/d365fin_md.md)] online kan godkjenne gruppemedlemmet. Hvis du vil ha mer informasjon, kan du se [Hurtigstart: Registrere et program i Microsoft Identity Platform](/azure/active-directory/develop/quickstart-register-app).
...
* I delen **API-tillatelser** legger du til tillatelser til [!INCLUDE[d365fin](includes/d365fin_md.md)]. Aktiver delegert tilgang til **Financials.ReadWrite.All** og **user_impersonation**.
...
> [!NOTE]
> Før du begynner må du kontakte representanten for mva-gruppen for å få følgende informasjon om deres [!INCLUDE[d365fin](includes/d365fin_md.md)]-leier:
>
...
1. Velg ikonet ![Lyspære som åpner Fortell meg-funksjonen](media/ui-search/search_small.png "Fortell hva du vil gjøre"), angi **Mva-gruppe**, og velg deretter den relaterte koblingen.
2. Hvis du vil definere selskapets mva-grupperolle, velger du **Medlem** og velger deretter **Neste**.
...
4. Legg til **Nettadresse for API** fra mva-grupperepresentanten. Vanligvis er URL-adressen formatert som følger: `https://api.businesscentral.dynamics.com/v2.0/[TENANT-ID]/[ENVIRONMENTNAME]`. Eksempel: `https://api.businesscentral.dynamics.com/v2.0/907869c3-b252-4aca-b9cb-17a15d25477b/UKRepresentative`. 
5. Legg til [!INCLUDE[d365fin](includes/d365fin_md.md)]-firmanavnet for mva-grupperepresentanten, for eksempel *CRONUS UK Ltd*.
6. Velg **Godkjenningstype**, velg **OAuth2**, og velg deretter **Neste**.
7. Skriv inn ID-en som er angitt av mva-grupperepresentanten, i feltet **Klient-ID**.
8. I feltet **Klienthemmelighet angitt av mva-grupperepresentant** skriver du inn hemmeligheten fra mva-grupperepresentanten.
9. I feltet **Godkjenningsendepunkt for OAuth 2.0** angir du *https://login.microsoftonline.com/common/oauth2*.
10. I feltet **Ressursnettadresse for OAuth 2.0** angir du *https://api.businesscentral.dynamics.com/*.
11. I feltet **Omdirigeringsnettadresse for OAuth 2.0** angir du *https://businesscentral.dynamics.com/OAuthLanding.htm*. 
12. Når du har angitt de ulike feltene, velger du **Neste**, og skriv deretter inn brukerlegitimasjonen som ble gitt av mva-grupperepresentanten.
13. Velg konfigurasjonen for mva-rapporten du bruker til å rapportere mva til myndighetene i landet.
