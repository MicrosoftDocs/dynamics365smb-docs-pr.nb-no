...
- Velg tekst
    ...
    Du kan kopiere tekst fra en rapport og lime den inn et annet sted, for eksempel en side i [!INCLUDE[prod_short](includes/prod_short.md)] eller Microsoft Word. Du kan for eksempel bruke musen til å trykke på venstre museknapp og holde nede der du vil begynne. Skyv musen for å merke et eller flere ord, setninger eller avsnitt. Trykk deretter på høyreklikknappen og velg **Kopier**. Du kan nå lime inn den merkede teksten der du vil ha den.
- Panorer dokumentet

    Du kan flytte det synlige området i rapporten i en hvilken som helst retning for å vise andre områder i rapporten. Panorering er nyttig når du har zoomet inn for å vise detaljer. Ved hjelp av musen kan du for eksempel trykke på og holde venstre museknapp hvor som helst i rapportforhåndsvisningen, og deretter flytte musen for å velge en del i rapporten.
